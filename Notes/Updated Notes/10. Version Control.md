...
- **AFR for Mirroring:**
  ...
  **Mirroring and Recovery Window:**
  ...
  +---------------+-------------------+-------------------+
                  ...
                  |   Window (e.g. 10h) 
  ```
  *Data is only at risk if the second drive fails during the replacement window after the first failure.*
...
- **Don't just model failure—model recovery!**
  ...
  **Common Mistakes:**
  - Only modeling failure, not recovery.
  - Forgetting to test the actual restore process.
  - Assuming backups are working without verification.
  - Deleting incrementals without combining them, risking unrecoverable data.
...
- **Git clone:**
  ...
  **Git Data Flow:**
  ...
  +----------------+                   +-------------+                      +-------------------+
  ```
  *You edit files, stage changes in the index, and commit them to the object database (history).*
